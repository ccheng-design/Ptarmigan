...
objs=rs.GetObjects("select Objects",filter=0,preselect=True)

layerNames=[]

for obj in objs:
    objlayers=rs.ObjectLayer(obj)
    layerNames.append(objlayers)

print(layerNames)
for obj in layerNames:
    rs.ObjectsByLayer(obj,True)
