...
namespace _Ptarmigan.Depreciated
{
    public class ImportVectorSRS : HeronComponent
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            DA.GetDataTree("Boundary", out boundaryTree);

            GH_Structure<GH_String> shpFilePathTree = new GH_Structure<GH_String>();
            DA.GetDataTree("File Path", out shpFilePathTree);
            ...
            ConcurrentDictionary<GH_Path, Geometry> geomDict = new ConcurrentDictionary<GH_Path, Geometry>();
            ...
            SpatialReference heronSRS = new SpatialReference("");
            ...
            int sourceSRSInt = short.Parse(heronSRS.GetAuthorityCode(null));
            ...
            for (int boundaryBranch = 0; boundaryBranch < boundaryCount; boundaryBranch++)
            {
                ...
                for (int index = 0; index < shpFilePathTree.DataCount; index++)
                {
                    ...
                    for (int iLayer = 0; iLayer < dataSource.GetLayerCount(); iLayer++)
                    {
                        ...
                        SpatialReference sourceSRS = new SpatialReference(Osr.SRS_WKT_WGS84_LAT_LONG);
                        ...
                        Envelope envelopeOgr = new Envelope();
                        ...
                        {
                            Geometry sourceOrigin = new Geometry(wkbGeometryType.wkbPoint);
                        ...
                        Geometry envelopeRing = new Geometry(wkbGeometryType.wkbLinearRing);
                        ...
                        ///Get the field names
                        FeatureDefn def = ogrLayer.GetLayerDefn();
                        ...
                        else
                        {
                            ...
                            Geometry clippingPolygon = Heron.Convert.MeshToMultiPolygon(boundaryMesh, heronSRSToUserSRSTransform);
                            ...
                            {
                                Geometry geomUser = feat.GetGeometryRef().Clone();
                                Geometry sub_geomUser;
        ...
        private string GetSpatialReferenceSRS(OSGeo.OGR.Layer layer, int iLayer, DataSource dataSource, SpatialReference sourceSRS)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    try
                    {
                        int sourceSRSInt = short.Parse(sourceSRS.GetAuthorityCode(null));
        ...
        {
            get { return new Guid("{C6DDDEA6-6D31-4A86-B67A-4437DF663FF4}"); }
        }
    }
}
